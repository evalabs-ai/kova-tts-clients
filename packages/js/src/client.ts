...
export const DEFAULT_BASE_URL = 'https://api.kova.ai/v1/tts';
...
export class KovaTTSClient {
    ...
    async *streamTTS(request: TTSRequest): AsyncGenerator<StreamEvent> {
        ...
        if (!response.body) {
            throw new KovaTTSProtocolError(
                'Streaming response did not include a body'
            );
        }
    ...
    private async postJson(
        ...
        try {
            ...
            throw new KovaTTSConnectionError(
...
export function serializeTTSRequest(
    ...
    return {
        ...
        ...(request.timestamps !== undefined
            ...
            : {}),
        ...(request.normalize_text !== undefined
...
export function parseSyncResponse(value: unknown): SyncTTSResponse {
    ...
    const response = value as {
        audio?: unknown;
        timestamps?: SyncTTSResponse['timestamps'];
    };
    ...
    if (response.timestamps && !Array.isArray(response.timestamps.words)) {
        throw new KovaTTSProtocolError(
            'Sync TTS response timestamps are invalid'
        );
    }
    return {
...
async function toResponseError(response: Response): Promise<Error> {
    ...
    return errorForStatus(
        response.status,
        `Kova TTS request failed with ${response.status}`,
        body
    );
}
